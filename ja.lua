...
-- Nametag with silver/gold gradient
local function v45(p5)
    ...
    RunService.RenderStepped:Connect(function(dt)
        n2 = (n2 + dt * 0.35) % 1
        -- Silver to gold cycling gradient
        local r = 0.83 + math.sin(n2 * 6.283) * 0.1
        local g = 0.69 + math.sin(n2 * 6.283 + 1) * 0.15
        local b = 0.22 + math.sin(n2 * 6.283 + 2) * 0.1
        TextLabel4.TextColor3 = Color3.new(
            math.clamp(r, 0.7, 1),
            math.clamp(g, 0.6, 0.9),
            math.clamp(b, 0.1, 0.4)
        )
    end)
end
